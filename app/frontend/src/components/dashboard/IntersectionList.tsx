      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <h3 style={{ fontSize: '1.05rem', fontWeight: 600, color: '#fff' }}>
            Tespit Edilen Kesişim ve Çaprazlama Noktaları Logu
          </h3>
        </div>
        <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>
          Toplam <strong>{intersections.length}</strong> Kesişim Bulundu
        </span>
      </div>
      ...
      {intersections.length === 0 ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
            {displayedIntersections.map((item) => {
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', color: '#38bdf8', fontWeight: 600 }}>
                      <Clock size={14} />
                      <span>{item.dateLabel} Saat {item.hourLabel}</span>
                    </div>
