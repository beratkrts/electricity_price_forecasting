...
from fetch_epias_data import fetch_water_energy_provision, get_tgt_token
...
def main():
    ...
    start_iso = os.getenv("WATER_ENERGY_START_DATE", default_start)
    end_iso = os.getenv("WATER_ENERGY_END_DATE", default_end)

    print(f"🔑 Requesting TGT Token for EPİAŞ API...")
    tgt_token = get_tgt_token(username, password)

    print(f"🚀 Fetching Water Energy Provision data ({start_iso[:10]} to {end_iso[:10]})...")
    records = fetch_water_energy_provision(tgt_token, start_iso, end_iso)

    output_dir = PROJECT_ROOT / "data" / "api_trials"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"water_energy_provision_{start_iso[:10]}_{end_iso[:10]}.json"

    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(records, f, ensure_ascii=False, indent=4)

    print(f"✅ Water energy provision records fetched: {len(records)}")
