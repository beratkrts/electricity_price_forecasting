...
from fetch_epias_data import fetch_active_fullness, get_tgt_token
...
def main():
    ...
    start_iso = os.getenv("ACTIVE_FULLNESS_START_DATE", default_start)
    end_iso = os.getenv("ACTIVE_FULLNESS_END_DATE", default_end)

    print(f"🔑 Requesting TGT Token for EPİAŞ API...")
    tgt_token = get_tgt_token(username, password)

    print(f"🚀 Fetching Active Dam Fullness data ({start_iso[:10]} to {end_iso[:10]})...")
    records = fetch_active_fullness(tgt_token, start_iso, end_iso)

    output_dir = PROJECT_ROOT / "data" / "api_trials"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_file = output_dir / f"active_fullness_{start_iso[:10]}_{end_iso[:10]}.json"

    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(records, f, ensure_ascii=False, indent=4)

    print(f"✅ Active dam fullness records fetched: {len(records)}")
